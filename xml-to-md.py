...
def normalize_numeric_input(s: str) -> str:
    """全角数字を半角に正規化し、それ以外はそのまま返す。
    例: '１' -> '1', '２' -> '2' など。空文字はそのまま返す。
    """
    if not s:
        return s
...
def normalize_yes_no(s: str) -> str:
    """全角の 'ｙ'/'ｎ' および全角大文字を半角に正規化し、小文字化して返す。
    例: 'ｙ' -> 'y', 'Ｎ' -> 'n'
    """
    if not s:
        return s
...
def process_amend_provision(xml_root):
    ...
    for amend_prov in amend_provs:
        ...
        # 改正規定内にNewProvision要素があれば、そちらを再帰的に処理
        new_provs = amend_prov.findall("NewProvision")
        if new_provs:
            for newp in new_provs:
                # NewProvision配下の構造要素を処理
                for part in newp.findall("Part"):
                    markdown_text += process_structure_element(part)
                for chapter in newp.findall("Chapter"):
                    markdown_text += process_structure_element(chapter)
                for section in newp.findall("Section"):
                    markdown_text += process_structure_element(section)

                # NewProvision直下の条を処理（見出しレベル3を使用）
                for article in newp.findall("Article"):
                    markdown_text += process_article(article, 3)

        # 改正規定直下の条（NewProvisionの外に直接置かれているArticle）を処理
...
def extract_suppl_provision(xml_root):
    ...
    # 附則内の章（Chapter）がある場合は先に処理
    for chapter in suppl.findall("Chapter"):
        markdown_text += process_structure_element(chapter)

    # 附則の条（Chapterの外に直接置かれているArticle）
...
def process_article(article, heading_level):
    ...
    if total_paragraphs > 0:
        for para in paragraphs:
            ...
            # 号を処理（項の次のレベルなのでインデントレベルは 1）
            items = para.findall("Item")
            for item in items:
                markdown_text += process_item(item, 1)

        # Article直下に号（Item）が存在する場合の処理
        # 一部の文書で項を飛ばして号が直接Article配下に置かれることがあるため
        # その場合も適切にインデントして出力する
        article_level_items = article.findall("./Item")
        if article_level_items:
            markdown_text += "\n"
            for item in article_level_items:
                markdown_text += process_item(item, 1)

    # 2. 項が存在しない場合（Article直下に号やその他の要素がある場合）の処理
    else: 
        # Article直下の要素を処理 (項が存在しない場合)
        
        # Listを処理
        for list_elem in article.findall("List"):
            markdown_text += process_list(list_elem, indent_level=0)
        ...
        # 図を処理
        for fig_struct in article.findall("FigStruct"):
            markdown_text += "\n"
            markdown_text += process_fig_struct(fig_struct)
        ...
        for style_struct in article.findall("StyleStruct"):
            markdown_text += "\n"
            markdown_text += process_style_struct(style_struct)
        
        # 記項目（NoteStruct）を処理
        for note_struct in article.findall("NoteStruct"):
            markdown_text += "\n"
            markdown_text += process_note_struct(note_struct)
        
        # 書式項目（FormatStruct）を処理
        for format_struct in article.findall("FormatStruct"):
            markdown_text += "\n"
            markdown_text += process_format_struct(format_struct)
        
        # 類（Class）を処理
        for class_elem in article.findall("Class"):
            markdown_text += "\n"
            markdown_text += process_class(class_elem)
        
        # 号を処理（項がない場合は、号が条の次のレベルとなるため、インデントレベルは 1）
        items = article.findall("Item")
        for item in items:
            markdown_text += process_item(item, 1)
        
        # ParagraphSentenceが存在しない場合（Article直下のSentence）
        sent = article.find("./Sentence")
        if sent is not None:
            text = normalize_text(extract_text(sent))
            markdown_text += f"{text}\n"

    # 3. 共通の末尾要素を処理 (項の有無に関わらず Article直下)
...
def render_table(table_elem, indent_level=0):
    ...
    for row_elem in table_elem.findall("TableRow"):
        ...
        for col in cols:
            ...
            # colspanの処理（Markdownテーブルでは単純なcolspanは表現できないため、ここではセルを結合する）
            colspan = int(col.get("colspan", 1))
            if colspan > 1:
                # 結合されたセルに対して後続のセル位置にマーカーを挿入
                for i in range(1, colspan):
                    if col_idx + i < max_cols:
                        current_row[col_idx + i] = "#COLSPAN_MERGED#"
            
            col_idx += colspan # 次のカラム位置に進む

        # Noneを空文字列に変換、#COLSPAN_MERGED#を無視
        final_row = []
        for cell in current_row:
            if cell != "#COLSPAN_MERGED#":
                final_row.append(cell if cell is not None else "")
        
        # 行の長さを max_cols に合わせる
        final_row = final_row + [""] * (max_cols - len(final_row))
    ...
    for row_idx, row in enumerate(grid):
        processed_row = []
        # 行の長さを max_cols に合わせるためにスライスとパディングを行う
        row_cells = row[:max_cols] + [""] * (max_cols - len(row)) 
...
if __name__ == "__main__":
    while True:
        ...
        mode_raw = input("選択してください (1/2/9): ").strip()
