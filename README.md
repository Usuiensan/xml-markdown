...
- ファイル名に施行日が付与されない可能性があります（XMLに施行日情報が含まれない場合）
- ローカルXMLファイルは e-Gov法令API の形式（`law_data_response` ラッパー付き）である必要があります
...
| **ファイル名形式** | 法令名_施行日.md | 法令名_施行日?.md | 法令名_施行日.md |
| **ネットワーク必須** | ✅ 必須 | ❌ 不要 | ✅ 必須 |
| **処理速度** | 遅い（API呼び出し有） | 速い（ローカル処理） | 中程度 |
| **利用場面** | 単発の変換 | ダウンロード済みXMLの処理 | 複数法令の一括処理 |
